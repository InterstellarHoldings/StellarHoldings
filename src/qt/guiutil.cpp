...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("interstellarholdings"))
        return false;
...
{
    // Convert interstellarholdings:// to interstellarholdings:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("interstellarholdings://"))
    {
        uri.replace(0, 12, "interstellarholdings:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "InterstellarHoldings.lnk";
}
...
{
    return GetAutostartDir() / "InterstellarHoldings.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        optionFile << "Name=InterstellarHoldings\n";
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("InterstellarHoldings-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  InterstellarHoldings-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("InterstellarHoldings-Qt"));
