...
#if defined(NDEBUG)
# error "InterstellarHoldings cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "InterstellarHoldings Signed Message:\n";
...
bool CTransaction::GetCoinAge(CTxDB& txdb, const CBlockIndex* pindexPrev, uint64_t& nCoinAge) const
{
	...
	BOOST_FOREACH(const CTxIn& txin, vin)
	{
		...
		if (pindexPrev->nHeight > 4000 && block.GetBlockTime() + nStakeMinAge > nTime) {
...
#ifdef ENABLE_WALLET
// InterstellarHoldings: attempt to generate suitable proof-of-stake
...
{
	RenameThread("InterstellarHoldings-loadblk");
